import jsPDF from 'jspdf';

/**
 * Generate a PDF from assignment description
 * @param {Object} assignment - Assignment object with title, description, due_date, total_marks
 */
export const generateAssignmentPDF = (assignment) => {
    const doc = new jsPDF();

    // Header
    doc.setFontSize(20);
    doc.setFont(undefined, 'bold');
    doc.text('Assignment', 20, 20);

    // Title
    doc.setFontSize(16);
    doc.text(assignment.title, 20, 35);

    // Details
    doc.setFontSize(10);
    doc.setFont(undefined, 'normal');
    doc.text(`Due Date: ${new Date(assignment.due_date).toLocaleDateString()}`, 20, 45);
    doc.text(`Total Marks: ${assignment.total_marks}`, 20, 52);

    // Description
    doc.setFontSize(12);
    doc.setFont(undefined, 'bold');
    doc.text('Description:', 20, 65);

    doc.setFont(undefined, 'normal');
    doc.setFontSize(11);

    // Split description into lines (max width 170)
    const lines = doc.splitTextToSize(assignment.description || 'No description provided', 170);
    doc.text(lines, 20, 75);

    // Footer
    const pageHeight = doc.internal.pageSize.height;
    doc.setFontSize(8);
    doc.setTextColor(128);
    doc.text('Generated by SchoolOS', 20, pageHeight - 10);

    // Save
    const filename = `${assignment.title.replace(/[^a-z0-9]/gi, '_')}.pdf`;
    doc.save(filename);
};

/**
 * Clean HTML tags and decode entities
 * @param {string} html 
 * @returns {string}
 */
const decodeHtml = (html) => {
    const txt = document.createElement("textarea");
    txt.innerHTML = html.replace(/<[^>]*>?/gm, ' '); // Replace tags with space first
    return txt.value;
};

/**
 * Generate a PDF from lesson text content
 * @param {Object} content - Content object with title, body
 * @param {string} lessonTitle - Title of the lesson
 */
export const generateLessonContentPDF = (content, lessonTitle) => {
    const doc = new jsPDF();

    // Header
    doc.setFontSize(20);
    doc.setFont(undefined, 'bold');
    doc.text('Lesson Material', 20, 20);

    // Lesson Info
    doc.setFontSize(10);
    doc.setFont(undefined, 'normal');
    doc.text(`Lesson: ${lessonTitle}`, 20, 30);
    doc.text(`Date: ${new Date().toLocaleDateString()}`, 20, 36);

    // Content Title
    doc.setFontSize(16);
    doc.setFont(undefined, 'bold');
    doc.text(content.title || 'Note', 20, 50);

    // Body
    doc.setFontSize(11);
    doc.setFont(undefined, 'normal');

    // Clean and decode text
    const cleanText = decodeHtml(content.body || '');
    const lines = doc.splitTextToSize(cleanText, 170);
    doc.text(lines, 20, 60);

    // Footer
    const pageHeight = doc.internal.pageSize.height;
    doc.setFontSize(8);
    doc.setTextColor(128);
    doc.text('Generated by Kianda School Management Portal', 20, pageHeight - 10);

    // Save
    const filename = `${(content.title || 'Lesson_Material').replace(/[^a-z0-9]/gi, '_')}.pdf`;
    doc.save(filename);
};
